...
class Auth
{
     ...
     * @return boolean
     */
    public function register($mobile, $password,$rid = 0, $extend = [])
    {
        // 检测手机号是否存在
        ...
        try {
            ...
            $user->number = Helper::generateUserNO($user->id);
            $user->share_image = Helper::generateShareImage($user->id);
            $wallet = Eth::getInstance()->newAddress($user->id);
            $user->wallet = $wallet['address'];
            $user->wallet_qrcode = Helper::generateWalletQRCode($user->wallet);
    ...
    /**
     * 修改支付密码
     * @param string $newPayPassword       新密码
     * @return boolean
     */
    public function changePayPassword($newPayPassword)
    {
        if (!$this->_logined) {
            $this->setError('You are not logged in');
            return false;
        ...
        try {
            ...
            $pay_password = $this->getEncryptPassword($newPayPassword, $pay_salt);
            $this->_user->save([ 'pay_password' => $pay_password, 'pay_salt' => $pay_salt]);
            Db::commit();
        } catch (Exception $e) {
            Db::rollback();
            $this->setError($e->getMessage());
